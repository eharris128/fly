...
- **5-minute-floor clamp drift (R1 / KTD-C).** For a floor-cadence cron (`*/5 * * * *`) the claim always lands after the boundary, so the `max(cron_next, now+5min)` clamp wins every time and the run phase walks forward by the sweep latency each occurrence (9:00 → 9:05:07 → 9:10:14 …). Nothing breaks, but boundary-aligned expressions show drifting next-run times. Accept the drift, or snap to the cron boundary when it is ≥ (now + 5min − small epsilon).
