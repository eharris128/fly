...
- A fly *crash* (not clean shutdown) orphans the child, which is assumed to self-terminate on a stream-write error against the closed pipe (claude is Node — libuv ignores SIGPIPE, so this is best-effort, and a child deep in a long tool call may not write for minutes). Monitors default `retry_on_interrupt` on, so the restarted app re-dispatches while the orphan may still live: state-wise benign (the orphan's row is already closed; its verdict goes nowhere) but it briefly doubles spend and could collide on cwd side effects. Accepted; U7 observes the self-termination once.
