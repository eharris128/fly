...
- KTD6. **Renderer: xterm.js WebGL addon with a DOM-renderer fallback on context loss/failure; dispose WebGL for background panes.** WebGL needs the same GPU path WebKitGTK's DMABUF issues break on Linux, and live WebGL contexts are capped (~16). Allocate WebGL to the focused pane plus the most-recently-focused visible panes up to a cap (≈12, with hysteresis to avoid create/dispose thrash on tab/pane switches); all other panes, visible or not, use the DOM renderer.
  - *2026-08-08 addendum (perf-audit T4):* the eviction landed, simplified — WebGL for **every pane of the active tab** (the visible set is already bounded well under the context cap by the min-pane-size floor), disposed on tab/workspace switch; no MRU cap or hysteresis needed. `lib/renderer.ts` (pure rule) + `Terminal.svelte` (attach/dispose). Default renderer flipped `dom` → `auto` with this; v1 had shipped DOM-by-default because the eviction was unbuilt and all-panes WebGL blanked inactive panes.
