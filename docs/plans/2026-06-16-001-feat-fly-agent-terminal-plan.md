...
**In scope (this plan):** a single-window Tauri desktop app on Ubuntu with (1) PTY-backed terminal panes via xterm.js, (2) tabs + horizontal/vertical splits, one agent per pane, (3) agent-attention indicators + OS notifications driven by an authenticated hook channel with a generic OSC/BEL fallback, (4) session restore of layout, working directory, and optional scrollback. Minimal GUI — a tab bar, panes, and attention indicators, nothing more.
...
- Agent matrix beyond the v1 set (the rest of cmux's ~13 agents; Cursor CLI is currently hook-dark).
...
- R11. `fly hooks setup` configures supported agents (Claude Code, Codex, Gemini, opencode, Amp) to report attention events; agents without hooks fall back to generic OSC 9/777/99 and BEL detection from the PTY stream.
...
- KTD7. **Attention transport: a local Unix-domain socket plus a per-pane secret token (`FLY_PANE_TOKEN`) injected into each pane's environment; Claude Code/Codex use their `http`/command hooks, command-hook agents call a thin `fly notify` CLI.** The PTY is a trust boundary: an unauthenticated endpoint lets any local process spoof attention or enumerate panes. The token maps each callback to its originating pane. The token is ≥128 bits from a CSPRNG, compared in constant time, registered before the child starts, and rate-limited against brute force; the Unix socket verifies the peer UID (`SO_PEERCRED`). The optional loopback HTTP endpoint (for `http`-only hooks) binds `127.0.0.1` on an ephemeral port, stays off unless such an agent is configured, validates `Origin`/`Host` (anti-DNS-rebinding), and takes the token in a header. The token is inherited by all descendants of a pane and authorizes attention for that pane only — never cross-pane or privileged action (see Risks).
...
Grouped into phases. Phases are sequential; units within a phase are dependency-ordered. U13–U14 (cross-cutting substrate) were added during deepening; their IDs are identity, not order — they sequence into Phases A and D respectively, as their notes state.
...
- **Approach:** On pane spawn, generate a per-pane secret token and inject `FLY_PANE_TOKEN` and `FLY_SOCKET_PATH` into the pane environment. Run a Unix-domain socket server (not TCP), plus an optional loopback HTTP endpoint for agents that only support `http` hooks (Claude Code/Codex), enforcing the same token check. Message schema `{token, reason: question|permission|finished|error, title?, body?}`. Validate the token → resolve the pane; reject unknown/missing tokens and malformed payloads. Map `reason` to an attention signal into U7. Remove the socket and invalidate the token on pane close / app exit. The token is ≥128-bit CSPRNG, compared in constant time, and registered before the child starts; repeated invalid tokens are throttled/locked out. The Unix socket checks the peer UID (`SO_PEERCRED`). The loopback HTTP endpoint binds `127.0.0.1` on an ephemeral port, is absent unless an `http`-hook agent is configured, rejects mismatched `Origin`/`Host`, and reads the token from a header (never a query string).
...
- **Test scenarios:**
  ...
  - A cross-origin POST with a wrong/absent `Origin` or `Host` header is rejected even without a token, and the HTTP endpoint is absent when no `http`-hook agent is configured.
...
- **Goal:** Configure each supported agent to report attention, with a CLI for command-hook agents.
- **Requirements:** R11
- **Dependencies:** U8
- **Files:** `src-tauri/src/cli/mod.rs`, `src-tauri/src/cli/hooks_setup.rs`, `src-tauri/src/cli/notify.rs`, `src-tauri/src/cli/agents/` (`claude.rs`, `codex.rs`, `gemini.rs`, `opencode.rs`, `amp.rs`)
- **Approach:** `fly` CLI subcommands of the app binary. `hooks setup [--agent ...]` idempotently writes each agent's config to call back: Claude Code → `~/.claude/settings.json` `http` hook (Notification/Stop/PermissionRequest) to the loopback endpoint; Codex → `~/.codex/hooks.json` command/http (Stop/PermissionRequest), noting its one-time trust review; Gemini → `settings.json` command hook (Notification `ToolPermission` / `AfterAgent`) invoking `fly notify`; opencode → community hook invoking `fly notify`; Amp → set `AMP_FORCE_BEL=1` in the pane env (BEL handled by U10). `fly notify <reason> [--title --body]` reads `FLY_PANE_TOKEN` and `FLY_SOCKET_PATH` from the env and sends to the socket. The per-agent capability matrix is documented; an unknown agent gives a clear error. Map Claude Code `Notification.notification_type` (`permission_prompt` / `idle_prompt`) and `Stop` to reasons. Config writes merge (read-modify-write) under a stable `fly`-owned marker, preserve unknown keys, and back up each file before first modification; hooks record the absolute, canonical `fly` binary path (never a `PATH`-relative name) and build command strings safely. A `fly hooks teardown` (and the `.deb` `postrm`) removes only `fly`-owned blocks on uninstall.
- **Patterns to follow:** cmux `hooks setup` / `notify`.
- **Test scenarios:**
  - `setup --agent claude` writes the correct `http` hook block idempotently (re-run adds no duplicate).
  - `setup --agent gemini` writes a command hook invoking `fly notify`.
  - `fly notify` with valid env posts to the socket and raises attention.
  - `fly notify` with no env errors cleanly.
  - A Claude Code `Stop` event maps to the "finished" reason.
  - A Gemini `ToolPermission` notification maps to the "permission" reason.
  - An unknown `--agent` errors clearly.
  - Codex setup surfaces the trust-review note.
...
#### U10. Generic fallback detection (OSC + BEL + OSC 7 cwd)

- **Goal:** Detect attention from hookless agents and track live cwd by scanning the PTY stream.
- **Requirements:** R11 (fallback), R13 (cwd tracking), R16 (untrusted-signal handling)
- **Dependencies:** U2 (stream), U7 (attention), U13 (fallback toggle)
- **Files:** `src-tauri/src/scan/mod.rs`, `src-tauri/src/scan/osc.rs`, `tests/osc_scan.rs`
- **Approach:** An incremental scanner over PTY bytes (before/alongside forwarding to xterm.js) reassembles partial sequences across read chunks. It detects OSC 9 (`\e]9;…`), OSC 777 (`\e]777;notify;…`), OSC 99, and BEL (`0x07`) → low-confidence attention (scoped to when an agent is believed active, disable-able), and OSC 7 (`\e]7;file://…`) → update the pane's live cwd; OSC 133;A optionally marks idle. Notification OSCs are stripped from the display stream; OSC 7 passes through. Low-confidence signals are visually distinct from or rate-limited relative to Tier-1 hooks. The common no-escape-sequence path is allocation-free — forward the original buffer unmodified when no OSC byte is seen, allocating a stripped buffer only when an OSC is present. Tier-3/4 signals are treated as untrusted (KTD9) and rate-limited (≤1 low-confidence raise per pane per second), and their text is sanitized before it can reach a notification. The U4 throughput figure is re-validated with this scanner inline.
- **Patterns to follow:** cmux OSC 9/777/99 parsing; Ghostty/WezTerm OSC handling.
- **Test scenarios:**
  - OSC 9 in the stream raises low-confidence attention.
  - OSC 777 notify parses title/body.
  - BEL raises attention when an agent is active and is ignored at a bare prompt.
  - An OSC sequence split across two read chunks is reassembled.
  - OSC 7 updates the pane cwd.
  - The notification OSC is stripped from rendered output.
  - Disabling the BEL fallback suppresses it.
  - OSC 133;A marks the pane idle.
  - Throughput with the scanner inline stays within a stated margin of the U4 baseline, and a pure `yes`/binary flood with no escape sequences incurs near-zero added latency (allocation-free fast path).
  - A pane emitting BEL/OSC at high frequency is rate-limited to at most one low-confidence raise per second.
- **Verification:** a hookless agent emitting OSC 9 raises attention; pane cwd follows `cd`.
...
- **Dependencies:** U7, U8, U10, U13 (thresholds from config)
...
- **Agent hook APIs are young and drift.** Gemini lacks a "waiting for input" event; Cursor CLI is hook-dark; Codex requires a trust review; schemas may change. Mitigated by tiered detection (KTD9) plus the generic OSC/BEL fallback; treat per-agent support as versioned and keep the capability matrix current.
...
- **Loopback HTTP endpoint reach.** The optional `http`-hook endpoint is reachable by any local process and by web pages the user visits (DNS-rebinding / CSRF-to-localhost). The per-pane token is the only control; the bounded impact is forged attention / injected notification content, not code execution. Mitigated by KTD7 (ephemeral `127.0.0.1`, `Origin`/`Host` checks, off-by-default, token-in-header).
- **Unauthenticated PTY stream is a back door around hook auth.** Anything writing to a pane (a remote `ssh` peer, a malicious file, a dependency's output) can emit OSC/BEL to spoof attention or inject notification text. Mitigated by KTD9 (untrusted, low-confidence, rate-limited, visually distinct) plus notification-text sanitization (R16).
...
- v1 agent matrix is Claude Code, Codex, Gemini, opencode, and Amp plus the generic fallback. Which of the remaining cmux agents are worth adding, and when? (Cursor CLI is hook-dark today.)
