...
| `2026-08-07-002-feat-automation-dependencies` | **Automation dependencies** — `fly automation create --after <id> [--within <dur>]`: a dependent's cron occurrence fires only against a fresh, successful, not-yet-consumed upstream run (bounded wait, then an honest born-terminal `withheld` row naming why — never a green row over stale data); exactly-once consumption via `RunRow.upstreamRunId` stamped atomically with the claim; create-time chain-depth/cycle rejection; withholds ring the Automations alert path; additive feed/dashboard projection (`after`, `lastWithheldReason`, `waiting on upstream`). Since fly-dag-primitives G1 the gate also reads the upstream's *verdict*, not just its run status: a success carrying FAIL or DECLINED does not satisfy a dependent, so an upstream verdict can withhold (alerting) or decline (silent) the leg below it | `automations/{model,depend,mod}.rs`, `cli/automation.rs`, `lib.rs`, `feed/{wire,mod}.rs`, `lib/{automations,feed}.ts`, `HomeView.svelte` |
| `2026-08-08-001-feat-automation-update` | **`fly automation update`** — patch a stored automation in place (name/cron/timezone, agent prompt/model/effort/disposition, script content/interpreter/timeout, retry toggle) with patch semantics (absent = unchanged, closed-set `clear` list for explicit resets); refuses the load-bearing mutations: mode-kind switch, monitors, retired records, setting/changing `after` (clear-only, preserving the dependencies plan's KTD6 cycle argument), and `cwd` (confidentiality-guard interaction); in-flight runs untouched (claimed rows are self-stamped), cron recompute only-when-enabled, script content swapped write-new-then-swap | `automations/{mod,store}.rs`, `cli/automation.rs`, `lib.rs` |
| `2026-08-08-002-feat-fly-dag-primitives` | **fly primitives for the CVE-outreach DAG** (design note, G1–G9 — G1 built, G2–G9 proposed): **G1 verdict-gated non-monitor runs + the DECLINED outcome.** An opt-in `Automation.verdict_gated` (never inferred from a block's presence) makes a non-monitor agent run's Succeeded close parse the fenced `verdict` block and stamp it on the row *without* retiring (retiring stays monitor-only); `parse_verdict` gains a third outcome, `DECLINED` — "ran, correctly had nothing to do" — which a dependent reads as `DepDecision::Declined`: a silent born-terminal row that chain-propagates and deliberately does **not** alert, since a correct no-op is not a failure. Monitors filter DECLINED out of their retire path (KTD6), so `VERDICT_BLOCK_SPEC` and the U8 skill still list only PASS/FAIL and stay byte-identical. CLI: `create --verdict-gated`, `update --verdict-gated`/`--no-verdict-gated` (the off direction rides the closed-set `clear` list) | `automations/{model,depend,verdict,mod}.rs`, `cli/automation.rs` |
...
resume launch-dir relocation for a drifted session cwd (`b821ca4`); and the
editable per-pane labels centered over each pane on `leader R` (`1b23519`).
One landed unit has no row of its own: `stream/coalesce.rs`, the
performance-audit **T1** PTY-read batching, is described in `CLAUDE.md`'s
`stream/` bullet and in `docs/notes/2026-07-23-performance-audit-follow-ups.md`
— its sibling T2 is the poll-batching plan row above. The
...
- `docs/notes/` — one-off evaluations and research notes that are neither a
  ...
  **`2026-07-24-phone-drop-live-check.md` holds the phone drop's operator setup**
  ...
  indefinitely. Open as of 2026-08-07, with candidate fixes. And
  `2026-08-08-typing-latency-diagnosis.md` is the measured root-cause of the
  persistent typing delay (audit T2 + T4 stacked; T1 confirmed insufficient),
  including the re-measurement protocol its fix plans validate against.
