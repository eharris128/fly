...
- R7. Quick handoff passes the stock prompt as a positional argv token preceding the variadic `--add-dir` flag (a trailing positional would be swallowed as another directory — U4's runtime verification) so the fresh instance starts working with no further input (origin R6).
...
- **Approach:** Handlers follow the `split()` idiom: capture focused leaf key + pane id synchronously, `await` resolution, then re-check the source still exists and do all seeding + tree mutation in one synchronous block (Terminal reads `cwd`/`command` once at mount). Seed `cwdByLeaf[newKey]` from `target.sessionCwd ?? live cwd` and `handoffCommandByLeaf[newKey]` from `buildHandoffCommand`. Quick argv: `["claude", <stock prompt>, "--add-dir", <project dir>]`; guided argv omits the prompt. Pass `automationRunId: null`. For R6, generalize the existing `resumeNotice` toast into a reusable transient notice rather than adding a new surface.
...
- **Test scenarios:**
  - Covers AE1 (origin). `buildHandoffCommand(target, "quick")` → argv with the prompt positional before `--add-dir`, no permissions-skip flag; prompt string contains the transcript path and the read-recent-portion instruction.
  - `buildHandoffCommand(target, "guided")` → same argv without the prompt.
