...
// ~20 ms of renderer main-thread work per coalesced output flush — with a few
...
// live GL context per pane is a bounded resource (and, on the WebKitGTK
// engine fly originally shipped on, many contexts blanked inactive panes),
// so the policy bounds live contexts to the panes of one tab: attach while
// visible, dispose on hide. `renderer: "dom"` in config is the engine-agnostic
// escape hatch. Terminal.svelte holds the effectful half (attach/dispose
// against the xterm addon); this module holds the rule so it is testable
// without a browser.
